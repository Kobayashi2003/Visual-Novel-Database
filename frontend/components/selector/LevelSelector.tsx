...
export function LevelSelectorButton({ levelOptions, selectedLevel, setSelectedLevel, disabled, className }: LevelSelectorProps) {
  ...
  return (
    <div className={cn("select-none", containerStyle, className)}>
...
export function LevelSelectorSelect({ levelOptions, selectedLevel, setSelectedLevel, disabled, className }: LevelSelectorProps) {
  ...
  return (
    <Select value={selectedLevel} onValueChange={setSelectedLevel} disabled={disabled}>
      <SelectTrigger className={cn("select-none", triggerStyle, className)}>
        <SelectValue placeholder="Level" />
      </SelectTrigger>
      <SelectContent className={cn("select-none", contentStyle)}>
        {levelOptions.map((option) => (
          <SelectItem
            key={option.value}
            value={option.value}
